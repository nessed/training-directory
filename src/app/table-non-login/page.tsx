...
export default function Home() {
  ...
  useEffect(() => {
    //useEffect used so that the data only makes a call to supabase on the first render rather than everytime theres a re render in the app. the dependency array is empty because we dont want it to re call async in general just once in the start
    const supabase = createBrowserClient(
      //supabase client is created with the url and anon key
    ...
    const fetchData = async () => {
      ...
      if (error) {
        ...
        //formatted assigns local values to data from the db
            ...
            >
              {loading //if loading state is true then show skeleton loaders
                ? [...Array(5)].map(
                    (
                      _,
                      index //first an empty array of 5 undefined elements is generated, then it is spread out to be turned to 5 arrays. these arrays serve as the number of rows the table cells below will appear
                    ) => (
                ...
                : getFilteredTrainers().map(
                    (
                      trainer,
                      index //else if loading it renders the other version of rows
                    ) => (
                      <TableRow
                        data-hover
                        key={index}
                      ...
                      >
                        ...
                        <TableCell className="text-left border-blue-200">
                          <div className="cursor-pointer flex gap-2 text-blue-800 hover:text-blue-600 transition duration-200">
                            {index + 1}
                          </div>
                        </TableCell>
